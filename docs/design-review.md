...
|----|-----|-------|------------|-----------|
| **DR-1** | 🔴 | `drive.file` can't deliver a *shared* multiplayer Doc across users (per-app, per-user file access). | A branch's Drive folder is **one canonical location** (account / Shared Drive), collaborators added as editors; each does a one-time Picker **"adopt"** per Doc. | S3, INV-5, constraint #2, W0/W3, open-risk #6 |
...
amended **before** implementation, so the invariants (now INV-1…INV-8), UAT cases (UAT-A…E), and
